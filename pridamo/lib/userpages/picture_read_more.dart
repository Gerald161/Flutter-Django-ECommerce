...
class picture_read_more extends StatefulWidget {
  var particular_ad;

  picture_read_more({Key key, @required this.particular_ad})
      : super(key: key);

  @override
  _picture_read_moreState createState() => _picture_read_moreState();
}

class _picture_read_moreState extends State<picture_read_more> {
  Map data = {};

  Future picture_read_more_future;
  ...
  Future get_picture_ad_initial_values() async {
    ...
    if(that_ad.length != 0){
      data['image'] = that_ad[0]['fields']['image'];
  ...
  get_initial_comments()async{
    await get_picture_ad_initial_values();
