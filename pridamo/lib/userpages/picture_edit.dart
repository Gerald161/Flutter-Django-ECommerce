...
class picture_ad_edit extends StatefulWidget {
  var particular_ad;

  var deletion_index;

  picture_ad_edit({Key key, @required this.particular_ad, @required this.deletion_index})
      : super(key: key);

  @override
  _picture_ad_editState createState() => _picture_ad_editState();
}

class _picture_ad_editState extends State<picture_ad_edit> {
  Map data = {};
  ...
  Future<Map> save_change_with_images(product_slug, product_name, description, price, phone_number, location, region, category, subcategory) async {
    final SharedPreferences prefs = await SharedPreferences.getInstance();

    final String token = prefs.getString('token');

    var request = http.MultipartRequest('POST', Uri.parse('https://pridamo.com/app_picture_edit/${product_slug.toString().toLowerCase()}?additional_image=True'));
  ...
  getImage() async {
    ...
    if(image != null){
      setState(() {
        _image = File(image.files.first.path);
        picture_taken = true;
      });
    }
  }
  ...
  Future get_picture_ad_initial_values() async {
    final SharedPreferences prefs = await SharedPreferences.getInstance();
    ...
    data['image'] = "https://media.pridamo.com/pridamo-static/${that_ad[0]['fields']['image']}";
...
//    expiration_time = "Expires on ${day_month_and_year} at ${twenty_four_hour_time}";
  ...
  Future delete_particular_image(index, number_of_images) async {
    ...
    if(what_it_said['status'] == 'deleted'){
      ...
      Navigator.pushReplacement(
        ...
        builder: (context) => picture_ad_edit(particular_ad: widget.particular_ad, deletion_index: widget.deletion_index),
      ),
      );
    }
  }
  ...
  Future _picture_edit_future_instance;
  ...
  void initState() {
    ...
    super.initState();
    _picture_edit_future_instance = get_picture_ad_initial_values();
  }
                      ...
                      FlatButton(
                        onPressed: () async{
                          if(_formKey.currentState.validate()){
                            ...
                            save_change_with_images(data['product_slug'], data['product_name'], data['description'], data['price'], data['number'], data['location'], data['region'], data['category'], data['subcategory']);
                          }
                        },
                        textColor: Colors.blue,
                        child: Text(
